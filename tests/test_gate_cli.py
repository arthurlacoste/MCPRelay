...
def test_version_reads_root_version_file(tmp_path):
    ...
    assert result.stdout.strip() == f"Gate {expected_version}"
...
def test_update_command_uses_lifecycle_helpers(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(main_module, "perform_gate_update", lambda edge, stable, target_version=None: ("0.2.0", True, "- New CLI"))
...
def test_update_migration_error_prints_report_and_issue(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(main_module, "perform_gate_update", lambda edge, stable, target_version=None: (_ for _ in ()).throw(error))
